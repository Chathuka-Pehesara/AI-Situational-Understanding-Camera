...
def get_situation_details(situation):
    details = {
        "Distracted Walking": ("Distracted Walking", "pink", "≡ƒô▒"),
        "Working": ("Working", "green", "≡ƒÆ╗"),
        "Resting": ("Resting", "cyan", "≡ƒ¢î"),
        "Hurrying": ("Hurrying", "yellow", "≡ƒÅâΓÇìΓÖé∩╕Å"),
        "Normal Activity": ("Normal Activity", "blue", "≡ƒÜ╢ΓÇìΓÖé∩╕Å"),
        "Weapon Detected": ("Weapon Detected", "pink", "≡ƒö¬"),
        "Vehicle Loitering": ("Vehicle Loitering", "yellow", "≡ƒÅì∩╕Å"),
        "Animal Intrusion": ("Animal Intrusion", "green", "≡ƒÉê")
    }
    return details.get(situation, (situation, "cyan", "≡ƒöÄ"))

def get_risk_details(risk):
    details = {
        "High": ("High Risk", "pink", "ΓÜá∩╕Å"),
        "Medium": ("Medium Risk", "yellow", "≡ƒöö"),
        "Low": ("Low Risk", "green", "Γ£à")
    }
    return details.get(risk, (risk, "cyan", "Γ¥ô"))
...
def get_focus_details(score):
    try:
        ...
        val = 100
    if val >= 80:
        return (f"{val}%", "green", "≡ƒÄ»")
    elif val >= 50:
        return (f"{val}%", "yellow", "≡ƒÄ»")
    else:
        return (f"{val}%", "pink", "≡ƒÄ»")
...
def get_safety_details(score):
    try:
        ...
        val = 10
    if val >= 7:
        return (f"{val}/10", "green", "≡ƒ¢í∩╕Å")
    elif val >= 4:
        return (f"{val}/10", "yellow", "≡ƒ¢í∩╕Å")
    else:
        return (f"{val}/10", "pink", "≡ƒ¢í∩╕Å")
...
st.sidebar.title("ΓÜÖ∩╕Å System Control")
st.sidebar.markdown("---")

st.sidebar.subheader("≡ƒôÉ Zone Configuration")
...
mode = st.sidebar.radio(
    "Monitoring Mode",
    ["≡ƒö┤ LIVE MONITORING", "≡ƒ¢á∩╕Å SIMULATOR", "≡ƒôñ UPLOAD VIDEO"],
    index=0,
    help="≡ƒö┤ LIVE MONITORING reads from the physical pipeline CSV log. ≡ƒ¢á∩╕Å SIMULATOR auto-generates test scenarios. ≡ƒôñ UPLOAD VIDEO processes an uploaded video frame-by-frame."
)
...
if mode == "≡ƒ¢á∩╕Å SIMULATOR":
    ...
    if uploaded_file is not None:
        ...
        if not st.session_state.video_incidents:
            if st.sidebar.button("≡ƒöì Scan Video for Key Incidents", help="Perform a rapid background scan to populate the timeline."):
            ...
            if st.sidebar.button("≡ƒöä Rescan Video"):
                st.session_state.video_incidents = []
                st.rerun()
        ...
        with col1:
            if st.button("Γû╢∩╕Å Play"):
                st.session_state.video_playing = True
                st.rerun()
        with col2:
            if st.button("ΓÅ╕∩╕Å Pause"):
                st.session_state.video_playing = False
                st.rerun()
        with col3:
            if st.button("ΓÅ╣∩╕Å Reset"):
                st.session_state.video_playing = False
                st.session_state.video_frame_index = 0
                reset_tracker()
                st.session_state.current_processed_frame = None
...
st.sidebar.markdown("---")
if st.sidebar.button("≡ƒùæ∩╕Å Clear Event Log"):
...
<div class="header-wrapper">
    <div class="title-text">
        <span>≡ƒÄÑ</span> AI Situational Camera Dashboard
    </div>
...
with left_col:
    ...
    st.markdown('<div class="section-header">≡ƒÄÑ Live Camera Monitoring</div>', unsafe_allow_html=True)
...
with right_col:
    st.markdown('<div class="section-header">≡ƒôè Real-Time Metrics</div>', unsafe_allow_html=True)
    metrics_placeholder = st.empty()
    chart_placeholder = st.empty()

st.markdown('<div class="section-header" style="margin-top: 2rem;">≡ƒôï Recent System Events</div>', unsafe_allow_html=True)
# Render static timeline jumper outside the loop to avoid StreamlitDuplicateElementId
if mode == "≡ƒôñ UPLOAD VIDEO" and st.session_state.video_incidents:
...
if mode == "≡ƒôñ UPLOAD VIDEO" and st.session_state.temp_video_path and not st.session_state.video_playing and st.session_state.current_processed_frame is not None:
    if gemini_btn_placeholder.button("≡ƒö« Request On-Demand Gemini Vision Analysis", help="Audit the current paused frame using the Gemini Vision model to confirm the situation."):
        import cv2
        from detection.detector import detect_objects
        cap = cv2.VideoCapture(st.session_state.temp_video_path)
        cap.set(cv2.CAP_PROP_POS_FRAMES, st.session_state.video_frame_index)
...
while True:
    ...
    if mode == "≡ƒôñ UPLOAD VIDEO" and st.session_state.get("temp_video_path") and st.session_state.get("video_playing"):
        import cv2
        from detection.detector import detect_objects
        from detection.tracker import is_moving
            ...
            try:
                ...
                while cap.isOpened() and st.session_state.get("video_playing", False):
                    ...
                    <div class="explanation-block">
                        <div class="explanation-title">≡ƒÆí Behavior Explanation</div>
    ...
    if df is not None and not df.empty:
        ...
        if mode == "≡ƒôñ UPLOAD VIDEO" and st.session_state.gemini_manual_insight is not None:
        ...
        if mode == "≡ƒôñ UPLOAD VIDEO" and not st.session_state.video_metrics_history.empty:
            chart_df = st.session_state.video_metrics_history.set_index("Frame")
            chart_placeholder.line_chart(chart_df)
        ...
        <div class="gemini-insights-panel" style="border-left-color: #475569;">
            <div class="gemini-title">≡ƒö« Gemini Insights</div>
        ...
        no_events_html = """
        <div class="no-events">
            ≡ƒôí Waiting for camera events...<br/>
            <span style="font-size: 0.8rem; color: #475569; margin-top: 0.5rem; display: block;">
                Run the main pipeline or switch to '≡ƒ¢á∩╕Å Simulator' in the sidebar to generate mock data.
            </span>
        </div>
        """
